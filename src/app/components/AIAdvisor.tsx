...
export function AIAdvisor({ student }: AIAdvisorProps) {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  return (
    <div className="h-full flex flex-col relative bg-zinc-50/30">

      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="p-6 border-b border-zinc-100 flex items-center justify-between bg-white/50 backdrop-blur-md sticky top-0 z-20">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center text-white shadow-lg shadow-primary/20">
            <Bot className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-lg font-black text-zinc-900 tracking-tight font-serif">Academic Advisor</h2>
            <div className="flex items-center gap-2">
              <span className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">AI Agent • Institutional Node</span>
            </div>
          </div>
        </div>

        {/* Model switcher */}
        <div className="flex bg-zinc-100/80 p-1 rounded-xl border border-zinc-200/50 scale-90 sm:scale-100 origin-right">
          {([
        ...
        <button
          ...
          className="p-2.5 text-zinc-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors squishy-button"
          title="Clear History"
        >
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-8 custom-scrollbar">
        ...
        {(!pdfContext || isProcessing) && (
          <motion.div
            ...
            className="bg-white rounded-3xl p-8 border border-zinc-200 shadow-xl shadow-zinc-200/40 relative overflow-hidden"
          >
            ...
            <div className="relative z-10">
              <h3 className="text-xl font-black text-zinc-900 mb-2 font-serif">Initialize Advisor</h3>
              <p className="text-sm text-zinc-500 mb-8 font-medium">
                Upload your official documents to anchor the AI analysis in your specific curriculum.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                {([
                  ...
                  <div
                    key={type}
                    className={`relative border-2 border-dashed rounded-2xl p-6 text-center transition-all ${file || cached ? activeColor : 'border-zinc-200 hover:border-primary hover:bg-zinc-50'
                      }`}
                  >
                    {(file || cached) && (
                      <button
                        onClick={(e) => { e.stopPropagation(); removeFile(type); }}
                        className="absolute -top-3 -right-3 w-8 h-8 bg-white rounded-xl shadow-lg border border-zinc-100 flex items-center justify-center text-zinc-400 hover:text-red-500 transition-colors z-10 squishy-button"
                      >
                    ...
                    />
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3 ${file || cached ? iconColor : 'bg-zinc-100 text-zinc-400'}`}>
              ...
              {/* Emphasis selector */}
              {hasBulletin && (
                <div className="bg-zinc-50 rounded-2xl p-4 border border-zinc-200 mb-6">
                  <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-3">Target Emphasis</p>
                  <div className="flex flex-wrap gap-2">
                    {EMPHASIS_OPTIONS.map(emp => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${selectedEmphasis === emp
                            ...
                            : 'bg-white text-zinc-600 border border-zinc-200 hover:border-primary'
                          }`}
                      >
                        {emp}
                      </button>
                    ...
                    <input
                      ...
                      className="px-4 py-2 rounded-xl text-xs font-bold bg-white text-zinc-600 border border-zinc-200 w-32 focus:outline-none focus:border-primary"
                    />
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all ${!canProceed
                      ...
                      : 'bg-primary text-white shadow-xl shadow-primary/20 hover:brightness-110 hover:-translate-y-0.5 active:translate-y-0 squishy-button'
                    }`}
                >
                  {isProcessing ? (
                    <>
                    ...
                    <><Sparkles className="w-5 h-5 text-secondary" /><span>Initialize Academic Analysis</span></>
                  ) : (
                    <><FileText className="w-5 h-5" /><span>Extract Institutional Data</span></>
                  )}
                </button>
                ...
                {uploadStatus === 'success' && (
                  <div className="flex flex-col items-center gap-1 mt-2">
                    <button
                      ...
                      className="flex items-center justify-center gap-2 text-[10px] text-zinc-400 hover:text-primary transition-colors uppercase font-bold tracking-widest"
                    >
                      <Zap className="w-3 h-3 text-secondary" />
                      View Institutional Context
                    </button>
        ...
        <div className="flex flex-col gap-6">
          <AnimatePresence>
            {messages.map(message => (
              ...
              >
                <div className={`w-10 h-10 rounded-xl shrink-0 flex items-center justify-center shadow-md ${message.role === 'user' ? 'bg-primary text-white' : 'bg-zinc-900 text-white'
                  }`}>
                ...
                <div className={`max-w-[80%] rounded-2xl p-5 text-sm leading-relaxed shadow-sm ${message.role === 'user'
                  ...
                  {message.role === 'user' ? (
                    ...
                    <div className="prose prose-sm prose-zinc max-w-none prose-headings:font-black prose-headings:font-serif prose-strong:text-zinc-900 prose-strong:font-black">
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm]}
                        components={{
                          p: ({ ...p }) => <p className="mb-3 last:mb-0" {...p} />,
                          h1: ({ ...p }) => <h1 className="text-lg font-black mb-2" {...p} />,
                          h2: ({ ...p }) => <h2 className="text-md font-black mb-2" {...p} />,
                          h3: ({ ...p }) => <h3 className="text-sm font-black mb-2" {...p} />,
                          ...
                          strong: ({ ...p }) => <strong className="font-black text-zinc-900" {...p} />,
          ...
          {isTyping && messages[messages.length - 1]?.content === '' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-4">
              <div className="w-10 h-10 rounded-xl bg-zinc-900 flex items-center justify-center">
                {selectedModel === 'groq' ? <Sparkles className="w-5 h-5 text-secondary" /> :
                  selectedModel === 'bytez' ? <Zap className="w-5 h-5 text-secondary" /> :
                    <Bot className="w-5 h-5 text-primary" />}
              </div>
              <div className="bg-white border border-zinc-100 rounded-2xl rounded-tl-none px-6 py-4 shadow-sm flex flex-col gap-2">
                ...
                <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                  {selectedModel === 'groq' ? 'Processing Engine…' : 'Synthesizing Response…'}
                </span>
              </div>
            </motion.div>
          )}
      ...
      {/* ── Input dock ─────────────────────────────────────────────────────── */}
      <div className="p-6 bg-white border-t border-zinc-100 relative z-20">
        <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {suggestedPrompts.map(prompt => (
                <button
                  ...
                  className="px-4 py-2 bg-primary/5 text-primary text-xs font-bold rounded-full border border-primary/10 hover:bg-primary/10 transition-all flex items-center gap-1 group squishy-button"
                >
                  {prompt}
          ...
          <div className="flex gap-3">
            <input
              ...
              placeholder={pdfContext ? 'Query graduation requirements or course costs…' : 'Initialize institutional context…'}
              disabled={!pdfContext || isTyping}
              className="flex-1 h-14 px-6 bg-zinc-50 border border-zinc-200 rounded-2xl focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all font-bold text-zinc-700 disabled:opacity-50"
            />
            <button
              onClick={() => handleSend()}
              disabled={!input.trim() || !pdfContext || isTyping}
              className="w-14 h-14 bg-zinc-900 text-white rounded-2xl flex items-center justify-center hover:bg-zinc-800 disabled:opacity-50 transition-all shadow-lg squishy-button"
            >
            ...
            >
              ...
              <div className="p-6 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-black text-zinc-800 font-serif">Institutional Context</h2>
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Verify and refine extracted PDF data</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setIsEditing(e => !e)}
                    className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${isEditing ? 'bg-primary text-white' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'
                      }`}
                  >
                    {isEditing ? 'Confirm Changes' : 'Refine Text'}
                  </button>
                  <button
                    onClick={() => setShowDebugModal(false)}
                    className="w-10 h-10 rounded-full hover:bg-zinc-100 flex items-center justify-center text-zinc-400 transition-colors squishy-button"
                    aria-label="Close"
                  >
              ...
              <div className="flex p-2 bg-zinc-100/50 border-b border-zinc-200">
                {(['transcript', 'bulletin'] as const).map(tab => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all ${debugTab === tab ? 'bg-white text-primary shadow-sm' : 'text-zinc-500 hover:text-zinc-700'
                      }`}
                  >
                    {tab === 'transcript' ? 'Transcript Data' : 'Bulletin Specs'}
                  </button>
                ))}
              </div>

              {/* Text area */}
              <div className="flex-1 overflow-auto bg-zinc-950 font-mono text-[11px] leading-relaxed custom-scrollbar">
                {isEditing ? (
                  <textarea
                    ...
                    placeholder={`Edit ${debugTab} data…`}
                    spellCheck={false}
                  />
                ) : (
              ...
              {/* Modal footer */}
              <div className="p-4 border-t border-zinc-100 bg-zinc-50 flex justify-between items-center gap-4">
                <div className="flex-1">
                  <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">
                    {isEditing
                      ? 'Editing Live Data Stream'
                      : 'Verified Institutional Output'}
                  </p>
                </div>
                {isEditing && (
                  <button
                    ...
                    className="px-6 py-2 bg-primary text-white rounded-xl text-xs font-bold shadow-lg shadow-primary/20 hover:brightness-110 active:scale-95 transition-all flex items-center gap-2 squishy-button"
                  >
